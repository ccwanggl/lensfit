...
from optibench.knowledge.base import FormulaParam, OpticalFormula
...
afov_from_focal = OpticalFormula(
    id="afov_from_focal",
    name_cn="视角计算",
    expression="AFOV = (360/π) × arctan(s / 2f)  [degrees]",
    latex=r"AFOV = \frac{360}{\pi} \arctan\left(\frac{s}{2f}\right) \; [^{\circ}]",
    params=[
        FormulaParam("sensor", "传感器宽度", "mm", "传感器感光区域的物理宽度"),
        FormulaParam("focal", "焦距", "mm", "镜头焦距"),
    ],
...
depth_of_field = OpticalFormula(
    ...
    params=[
        ...
        FormulaParam("focus_distance", "对焦距离", "mm", "镜头对焦平面到被摄体的距离"),
    ],
    outputs=["near_limit", "far_limit"],
    principle="景深是指在像平面上保持可接受清晰度的物距范围。由超焦距公式推导，光圈越小（F值越大）、焦距越短、弥散圆直径越大，景深越大。",
    assumption="几何光学近似，基于弥散圆判据。弥散圆直径由人眼分辨能力或传感器像素尺寸决定。",
    domain="all",
...
pixel_accuracy = OpticalFormula(
    id="pixel_accuracy",
    name_cn="像素精度",
    expression="acc = pixel_size_um / (1000 × β)  [mm/px]",
    latex=r"\text{acc} = \frac{p_{\text{size}}}{1000 \cdot \beta} \; [\text{mm/px}]",
    params=[
        FormulaParam("pixel_size_um", "像元尺寸", "μm", "传感器单个像素的物理边长"),

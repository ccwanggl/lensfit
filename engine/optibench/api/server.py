"""FastAPI server for OptiBench engine."""
...
def _migrate_legacy_db(db_url: str) -> None:
    """Rename a LensFit-era ``lensfit.db`` next to the target path if the new
    database file does not exist yet. Alembic versions live inside the database,
    so a plain file rename completes the migration."""
    prefix = "sqlite:///"
    if not db_url.startswith(prefix) or ":memory:" in db_url:
        return
    from pathlib import Path

    db_path = Path(db_url[len(prefix):])
    legacy_path = db_path.with_name("lensfit.db")
    if db_path.exists() or not legacy_path.exists():
        return
    for src in (legacy_path, *legacy_path.parent.glob("lensfit.db-*")):
        dst = db_path.with_name(src.name.replace("lensfit.db", db_path.name, 1))
        os.rename(src, dst)
...
async def lifespan(app: FastAPI):
    ...
    # server is started with ``python -m optibench.api.server`` vs imported in
    # tests).
    ...
    db_url = getattr(app.state, "db_url", None) or "sqlite:///optibench.db"
    _migrate_legacy_db(db_url)
    ...
    if getattr(app.state, "mode", None) == "desktop":
        print(f"OPTIBENCH_API_KEY {_API_KEY}", flush=True)
...
_API_KEY = os.environ.get("OPTIBENCH_API_KEY") or os.urandom(32).hex()
...
def main():
    ...
    parser.add_argument("--db", type=str, default="sqlite:///optibench.db")

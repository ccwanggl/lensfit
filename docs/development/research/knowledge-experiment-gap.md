...
> 结论先行：未锚定知识共 **116 概念 + 52 公式**（2026-08-25 晚间快照；`瑞利判据` 已于当日补链至 diffraction 实验）；其中真正适合"补实验"的是 T1 子集（约 20 项），大量内容属于其他能力域或天然纯阅读。
...
| # | 提案实验 | 锚定的未覆盖笔记 | 复用基建 | 量级 |
|---|---|---|---|---|
| 1 | 黑体辐射扩展：维恩位移 + 斯特藩玻尔兹曼定律 | `wien-displacement-law`、`stefan-boltzmann-law` | blackbody 实验 | S |
...
| 4 | 高斯光束传播实验（束腰/瑞利范围/M²） | `gaussian-beam-waist`、`rayleigh-range`、`m2-beam-quality`、概念 `gaussian-beam`、`rayleigh-range`、`beam-quality-m2` | 1D 曲线族 | S/M |
| 5 | 全反射临界角实验 | `tir-critical-angle`、`tir-lens`(部分) | snell_refraction 扩展 | S |
| 6 | 光纤数值孔径与受光锥实验 | `fiber-na`、概念 `multi-mode-fiber`(部分) | 同上复用 | S |
| 7 | 量子效率—响应度关系实验 | `qe-responsivity-relation`、`responsivity`(公式) | 关系曲线 | S |
| 8 | 探测器信噪比预算实验 | `detector-snr`、`NEP`、`specific-detectivity`、概念 `noise-equivalent-power`、`读出噪声`、`动态范围` | 噪声瀑布图 | M |

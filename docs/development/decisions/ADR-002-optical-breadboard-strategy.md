...
用户希望把 LensFit 扩展成一个**物理光学实验室的数字孪生**：
...
- **波动/干涉/衍射实验**：保留 LensFit 原生 Python 求解器，避免把波动光学硬塞进几何追迹引擎导致物理失真。
- **领域模型隔离**：LensFit 的 `SceneGraph` 是稳定的产品域模型；`ray-optics` 的 JSON 只作为适配器内部表示，不允许泄漏到保存场景或前端载荷中。
...
| 中文本地化 | 部分 | 完整（UI 由 LensFit 提供） | 完整 | 完整 |
...
```text
LensFit SceneGraph -> Solver Adapter -> 第三方/原生引擎
```

禁止反向依赖：

```text
ray-optics scene JSON -> LensFit 域模型
```

即使未来替换 `ray-optics`，已保存的 LensFit 场景和前端载荷也应只需修改适配器即可继续生效。
...
- `SceneGraph` 中只能出现 LensFit 语义字段（如 `laser-monochrome`、`single-slit`、`screen`），不能出现 `SingleRay`、`SphericalLens`、`Detector` 等第三方对象名。
- 设备目录（`EquipmentSpec`）与求解器映射（`SolverMapping`）分离，避免目录模型被第三方字段污染。
- 波动光学可观测值（如 `fraunhofer_intensity`）由 LensFit 原生计算；几何布局可由 `ray-optics` 渲染；两者通过 `composer` 分层叠加，并明确标注“几何光线”与“波动强度”。
...
| LensFit 域模型耦合到 ray-optics JSON | 高 | 映射表隔离；测试保存场景不含第三方类型名 |

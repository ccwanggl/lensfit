...
> 状态：切片 A、B、C-0 已完成（2026-08-26）；C-1 作废（见 §3 勘误）；深层去重是否立项待评审决策
...
> **⚠️ 勘误（2026-08-26 开工勘察）**：`components/domain/DomainPageShell.tsx` 早已存在（18 行三栏布局，里程碑 `ffbf9f5` 引入），四个页面均已接入。原 C-1「新增共享壳」前提作废；2026-08-25 评审 M1 所述重复实为各页编排层内部的领域逻辑同构，非布局层缺失。
>
> 范围修正后：
> - **C-0 表征测试：✅ 已完成（2026-08-26）**——四页冒烟测试 + 全量 API mock 工厂（src/test-utils/mockApi.ts）+ 渲染辅助与全局 jsdom stubs；全套 vitest 63 用例通过、tsc/build 绿。
> - **深层去重（配置驱动表单等）：降级为待评估**——是否立项由用户在 C-0 验收时决策，本计划不再默认包含。

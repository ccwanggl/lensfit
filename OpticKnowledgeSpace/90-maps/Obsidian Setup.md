...
- **Excalidraw**：把 [[90-maps/Visual Index|可视化索引]] 中的 SVG 拖入白板，自己画变体。
- **Dataview**：在 [[90-maps/Visual Learning Toolkit|视觉学习工具箱]] 中生成按 `status`、`type`、`domain` 过滤的学习进度表。
- **Canvas**：打开 [[90-maps/Interactive Explorer|交互式探索器]] 提到的 `Learning Path.canvas` 拖拽章节卡片。

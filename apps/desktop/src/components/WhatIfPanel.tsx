...
function getSensitivityLabel(_baseline: UnifiedMatchResult[], key: string): { label: string; icon: React.ReactNode; color: string } {
  ...
  if (key === "working_distance_mm" || key === "target_width_mm") {
    return { label: "高影响", icon: <Flame size={12} />, color: "text-rose-500 dark:text-rose-400" };
  }
  if (key === "pixel_size_um") {
    return { label: "中影响", icon: <Zap size={12} />, color: "text-amber-500 dark:text-amber-400" };
  }
  return { label: "低影响", icon: <TrendingUp size={12} />, color: "text-slate-400 dark:text-slate-500" };
}
      ...
      {!activeParam ? (
            ...
            return (
              ...
              >
                <div>
                  ...
                  <span className="text-xs text-slate-400 dark:text-slate-500 ml-2">{val}{p.key === "pixel_size_um" ? "μm" : "mm"}</span>
                </div>
                <span className={`flex items-center gap-1 text-xs font-medium ${sens.color}`}>
                  {sens.icon}
                  {sens.label}
                </span>
              </button>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={handleBack}
              className="text-xs text-slate-400 dark:text-slate-500 hover:text-indigo-500"
            >
              返回
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 dark:text-slate-500 tabular-nums w-10 text-right">{paramDef?.min}</span>
            <input
              type="range"
              min={paramDef?.min}
              max={paramDef?.max}
              ...
              className="flex-1 h-2 bg-slate-200 dark:bg-slate-700 rounded-full appearance-none cursor-pointer accent-indigo-500"
            />
            <span className="text-xs text-slate-400 dark:text-slate-500 tabular-nums w-10">{paramDef?.max}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {isRunning ? (
              <span className="text-xs text-indigo-500 animate-pulse">计算中...</span>
            ) : (
              <span className={`text-xs font-medium ${diffCount > 0 ? "text-emerald-500" : diffCount < 0 ? "text-rose-500" : "text-slate-400"}`}>
          ...
          {whatIfResults.length > 0 && baselineResults.length > 0 && (
            <div className="p-3 rounded-lg bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
              <p className="text-xs text-slate-500 dark:text-slate-500 mb-2">Top 3 对比</p>
              <div className="space-y-1.5">
                {whatIfResults.slice(0, 3).map((r) => {
                  ...
                  return (
                    <div key={`${r.lens_id}-${r.detector_id}`} className="flex items-center justify-between text-xs">
                      <span className="text-slate-600 dark:text-slate-300 truncate max-w-[140px]">{r.lens_model}</span>

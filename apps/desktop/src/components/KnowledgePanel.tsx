...
import { BookOpen, ChevronDown, ChevronRight, Calculator, Shield, Lightbulb, Loader2 } from "lucide-react";
...
type TabKey = "formulas" | "constraints" | "trace";

export default function KnowledgePanel({ form, domain = "industrial", selectedResult }: Props) {
  ...
  const tabs: Array<{ key: TabKey; label: string; icon: React.ReactNode }> = [
    { key: "formulas", label: "公式库", icon: <Calculator size={14} /> },
    { key: "constraints", label: "约束库", icon: <Shield size={14} /> },
    { key: "trace", label: "推导解释", icon: <Lightbulb size={14} /> },
  ];
  ...
  return (
    <div className="mt-5 p-4 rounded-[10px] bg-slate-50/80 dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200">光学物理知识库</h3>
      </div>
